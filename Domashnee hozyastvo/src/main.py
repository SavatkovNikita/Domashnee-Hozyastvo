...
class DataExporter:
    def __init__(self, db_path='../storage.db'):  # ИСПРАВЛЕН ПУТЬ
        self.db_path = db_path
        self.output_dir = '../out'  # ИСПРАВЛЕН ПУТЬ
...
def main():
    ...
    if not os.path.exists('../storage.db'):
        ...
        from repositories.database import create_test_database
        create_test_database()

    # Экспортируем данные
    exporter = DataExporter('../storage.db')
